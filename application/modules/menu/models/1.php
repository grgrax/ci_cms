...
class menu_m extends CI_Model
{
	...
	function read_menus_for_ordering()
	{
        ...
        $this->set_max_level();
        $this->max_level;
        $this->db->select('id,level,name,parent_id,order');
        $this->db->from($this->table);
        $this->db->order_by("level,order", "asc");
        $query = $this->db->get();
		...
		foreach ($menus as $menu) {
			if(!$menu['parent_id']){
				$final_menus[$menu['id']]=$menu;
			}
			else
			{
		        $parent=$this->get_parent($menus,$menu['parent_id']);
/*	        	echo "<br/>b4";
				show_pre($final_menus);
*/		        if($parent){
		        	//show_pre($parent);
		        	$parent_id=$parent['parent_id']?$parent['parent_id']:$parent['id'];
					if($menu['id']==14){
						$final_menus[10]['children'][11]['children'][$menu['id']]=$menu;
					}
					else{
						$final_menus[$parent_id]['children'][$menu['id']]=$menu;
					}
		        }
		        else
					$final_menus[$menu['parent_id']]['children'][$menu['id']]=$menu;
/*	        	echo "<br/>after";
				show_pre($final_menus);
*/			}
		}
	...
	function get_parent($menus,$id){
		foreach ($menus as $menu) {
			if($menu['id']==$id){
				echo "<br>add child under parent_id: $id";
				return $menu;
			}
		}
	}
	...
	function add_menu(){
        ...
        $this->db->order_by("level,order", "asc");
        $query = $this->db->get();
        $this->menus[]=$query->result_array();
        show_pre($this->menus);
		$final_menus=[];
		foreach ($this->menus as $menu) {
			if(!$menu['parent_id']){
				$final_menus[$menu['level']][$menu['id']]=$menu;
			}
			else
			{
		        $parent=$this->get_parent($menus,$menu['parent_id']);
...
*/			}
	...
	function add_child_menu($parent,$child,$level){
		foreach ($this->menus as $menu) {
			$final_menus[$level][$parent]['children']=$child;
		}	
	}

	function _read_menus_for_ordering()
	{
		...
		foreach ($menus as $menu) {
			...
			{
				$final_menus[$menu['parent_id']]['children'][]=$menu;
			}
		}
		// show_pre($final_menus);
		return ($final_menus);
